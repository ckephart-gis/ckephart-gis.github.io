...
// sidebar built in Experience Builder, reimplemented here with Leaflet
// and synthetic parcel attributes for portfolio purposes. Category
// names and quick-link buttons mirror the real application's structure.
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var categories = [
    'Property', 'Ownership', 'Sales', 'Zoning', 'Sketch',
    'School Districts', 'Food Pantries Near By', 'Parks Near By',
    'Political Districts', 'Sanitation Services', 'Winter Road Services',
    'Fire District', 'Miscellaneous', 'Flood Zone & FEMA Information'
  ];
  var quickLinks = ['Assessor Card', 'Google Map', 'Street View', 'Bing Map', 'Permitting', 'Land Records'];
    ...
    for (var c = 0; c < cols; c++) {
      ...
      (function (poly, parcelId, owner, zone, acreage) {
        ...
        poly.on('click', function () {
          ...
          var quickLinkHtml = quickLinks.map(function (label) {
            return '<span class="ql-btn">' + label + '</span>';
          }).join('');

          var categoryHtml = categories.map(function (cat) {
            return '<div class="accordion-row">▸ ' + cat + '</div>';
          }).join('');
          ...
          sidebar.innerHTML =
            ...
            '<div class="ql-row">' + quickLinkHtml + '</div>' +
            '<div class="accordion-list">' + categoryHtml + '</div>';
        });
